...
namespace Util {
    ...
    template <typename T, int R, int C> void TopTenStream(std::ostream& os, Eigen::Matrix<T, R, C> m) {
        ...
        std::sort(vals_and_coords.rbegin(), vals_and_coords.rend(), [](auto a, auto b) { return std::get<0>(a) < std::get<0>(b); });
    ...
    template <typename T, int R, int C> void CompareTopTen(std::ostream& os, Eigen::Matrix<T, R, C> &m, Eigen::Matrix<T, R, C> &other) {
        const size_t count = 10;
    ...
    template <typename T, int R, int C> void TopTenDifferences(std::ostream& os, Eigen::Matrix<T, R, C> &m, Eigen::Matrix<T, R, C> &other, std::function<std::string(size_t, size_t)> describer) {
        ...
        std::sort(vals_and_coords.rbegin(), vals_and_coords.rend(), [](auto a, auto b) { return std::abs(std::get<0>(a) - std::get<2>(a)) < std::abs(std::get<0>(b) - std::get<2>(b)); });
    ...
    template <typename T, int R> void DifferencesForIndicies(std::ostream& os, const std::vector<size_t> &indicies, Eigen::Matrix<T, R, 1> &m, Eigen::Matrix<T, R, 1> &other, std::function<std::string(size_t, size_t)> describer) {
