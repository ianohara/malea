...
namespace Vs {
    ...
    public:
        ...
        inline std::vector<size_t> GetIncomingNodesFor(size_t node_idx) {
            ...
            for (size_t from_node_idx : GetNodesForLayer(from_layer_idx)) {
                if (connections.count(from_node_idx)) {
                    ...
                    if (std::find(from_node_connections.begin(), from_node_connections.end(), node_idx) != std::end(from_node_connections)) {
        ...
        inline std::vector<size_t> GetOutgoingNodesFor(size_t node_idx) {
            return connections.at(node_idx);
        }
        ...
        inline bool GetAreConnected(size_t from_idx, size_t to_idx) {
            ...
            return (std::find(outgoing_connections.begin(), outgoing_connections.end(), to_idx) != std::end(outgoing_connections));
        }

        inline void SetBiasForLayer(size_t layer_idx, WVal bias) {
            biases[layer_idx] = bias;
        }

        inline void DeleteBias(size_t layer_idx) {
            biases.erase(layer_idx);
        }

        inline WVal GetBias(size_t layer_idx) {
            return biases.at(layer_idx);
        }
        ...
        size_t GetOptimizedParamsCount() {
            return GetOptimizedWeightCount() + GetOptimizedBiasCount();
        }
        ...
        void SummarizeObjDelNode(std::ostream &stream) {
            ...
            for (size_t layer_idx = 1; layer_idx < GetLayerCount(); layer_idx++) {
                ...
                for (size_t node_idx : GetNodesForLayer(layer_idx)) {
                    stream << "    n=" << node_idx << " del_O/del_i=" << del_objective_del_node_input[node_idx] << " (node_input=" << node_inputs[node_input_idx++] << ")" << " (node_output=" << node_output_values[node_idx] << ")" << std::endl;
                }
            }
        }
        ...
        void SummarizeAllForwardDebugInfo(std::ostream &stream, const IOVector &input, const bool include_input_layer) {
            ...
            for (size_t layer_idx = 0; layer_idx < GetLayerCount(); layer_idx++) {
                stream << "Layer " << layer_idx << " activation function: " << activation_functions[layer_idx]->Describe() << std::endl;
    ...
    Vs::GradVector CalculateNumericalGradient(
        const std::shared_ptr<Vs::Network> network,
        const Vs::IOVector &input,
