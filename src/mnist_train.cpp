...
int main(int arg_count, char** args) {
    ...
    for (size_t training_idx=0; training_idx < training_count; training_idx++) {
        ...
        Vs::GradVector sample_gradient = mnist_network->WeightGradient(this_image_input_format, this_one_hot, objective_fn);
        ...
        if (count_in_batch >= batch_size || training_idx >= training_count) {
            ...
            std::cout << "Completed step " << optimizer.GetStepCount() << " batch error / # samples in batch=" << batch_error / count_in_batch << std::endl;
